...
const LeftMenu = ({ generalSectionLinks, pluginsSectionLinks }) => {
    ...
    return ( <
        ...
        <
        NavBrand workplace = "Workplace"
        title = "Strapi Dashboard"
        icon = { < img src = { menuLogo }
        ...
        icon = { < Write / > } > { formatMessage({ id: 'content-manager.plugin.name', defaultMessage: 'Content manager' }) } <
        /NavLink>
                ...
                NavSection label = "Plugins" > {
                    pluginsSectionLinks.map(link => {
                        ...
                        return ( <
                            NavLink to = { link.to }
                            key = { link.to }
                            icon = { < Icon / > } > { formatMessage(link.intlLabel) } <
                            /NavLink>
                        );
                    })
                } <
                ...
                NavSection label = "General" > {
                    generalSectionLinks.map(link => {
                        ...
                        return ( <
                            ...
                            to = { link.to }
                            key = { link.to }
                            icon = { < LinkIcon / > } > { formatMessage(link.intlLabel) } <
                            /NavLink>
                        );
                    })
                } <
        ...
        <
        NavUser id = "main-nav-user-button"
        ref = { buttonRef }
        onClick = { handleToggleUserLinks }
        initials = { initials } > { userDisplayName } <
        /NavUser> {
        userLinksVisible && ( <
            LinkUserWrapper onBlur = { handleBlur }
            padding = { 1 }
            shadow = "tableShadow"
            background = "neutral0"
            hasRadius >
            ...
            Stack size = { 0 } >
            <
            LinkUser onClick = { handleToggleUserLinks }
            to = "/me" >
            <
            Typography > {
                formatMessage({
